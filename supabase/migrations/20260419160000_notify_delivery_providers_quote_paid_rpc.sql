-- After a builder pays the delivery quote (status delivery_quote_paid), notify all active
-- delivery providers in-app (+ queue row). Idempotent via delivery_quote_providers_alerted_at.

ALTER TABLE public.delivery_requests
  ADD COLUMN IF NOT EXISTS delivery_quote_providers_alerted_at TIMESTAMPTZ;

COMMENT ON COLUMN public.delivery_requests.delivery_quote_providers_alerted_at IS
  'When automatic provider alerts were sent after Paystack quote payment (idempotency).';
...
REVOKE ALL ON FUNCTION public.notify_delivery_providers_quote_paid(uuid) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.notify_delivery_providers_quote_paid(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.notify_delivery_providers_quote_paid(uuid) TO service_role;

COMMENT ON FUNCTION public.notify_delivery_providers_quote_paid(uuid) IS
  'Sends in-app delivery_request notifications + queue rows to all active drivers after quote payment; safe to call multiple times.';
