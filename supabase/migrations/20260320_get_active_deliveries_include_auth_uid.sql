-- ============================================================
-- Include delivery_requests / purchase_orders where provider_id or
-- delivery_provider_id = auth.uid() so no in-transit order is missing
-- when some rows are linked by user id and others by delivery_providers.id
...
BEGIN
  ...
  -- Include BOTH: provider_id = delivery_providers.id AND provider_id = auth.uid() so no order is missing
  WITH from_dr AS (
...
COMMENT ON FUNCTION public.get_active_deliveries_for_provider() IS
  'Returns active (scheduled + in_transit) deliveries for current provider. Includes rows where provider_id/delivery_provider_id = delivery_providers.id OR auth.uid() so no order is missing.';
