-- Comprehensive SECURITY DEFINER View Security Fix
-- This migration detects and removes all SECURITY DEFINER views to resolve SUPA_security_definer_view linter error

-- Create audit table for tracking security fix operations if it doesn't exist
...
-- Function to detect and log SECURITY DEFINER views
...
-- Function to safely drop SECURITY DEFINER views
...
-- Function to verify no SECURITY DEFINER views remain
...
-- Event trigger function to prevent future SECURITY DEFINER view creation
...
-- Execute the cleanup
SELECT * FROM drop_security_definer_views();
...
BEGIN
    ...
    INSERT INTO master_rls_security_audit (event_type, access_reason, additional_context)
    VALUES (
        'FINAL_SECURITY_VERIFICATION',
        CASE 
            WHEN remaining_views = 0 THEN 'SUCCESS: No SECURITY DEFINER views remain'
            ELSE format('WARNING: %s SECURITY DEFINER views still exist', remaining_views)
        END,
        jsonb_build_object(
