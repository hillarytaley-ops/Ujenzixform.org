-- ============================================================
-- Expand RLS Policy for delivery_requests to allow ALL delivery providers
-- to see ALL pending/requested/assigned requests (regardless of provider_id)
...
-- to see ALL pending/requested/assigned requests (so they can accept them)
CREATE POLICY "delivery_requests_provider_view_pending" ON delivery_requests
FOR SELECT TO authenticated
USING (
  -- ALL pending/requested/assigned requests are visible to ALL delivery providers (for first-come-first-served)
...
-- Also ensure delivery providers can update pending/requested/assigned requests (to accept them)
...
USING (
  -- Can update pending/requested/assigned requests (to accept them)
  status IN ('pending', 'requested', 'assigned')
  OR
