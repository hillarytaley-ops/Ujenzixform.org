...
-- DROP ALL EXISTING POLICIES (including the ones we just created)
...
DROP POLICY IF EXISTS "suppliers_builders_directory_only" ON public.suppliers;
...
CREATE POLICY "suppliers_admin_ultra_secure_access" 
ON public.suppliers 
FOR ALL 
USING (
  EXISTS (
...
CREATE POLICY "suppliers_self_only_ultra_secure" 
ON public.suppliers 
FOR ALL 
USING (
  EXISTS (
...
-- 3. Builder Ultra-Limited Access Policy - ONLY company name and verification, NO CONTACT DATA
CREATE POLICY "suppliers_builders_name_only_ultra_secure" 
ON public.suppliers 
FOR SELECT 
USING (
  EXISTS (
...
CREATE OR REPLACE FUNCTION get_supplier_contact_ultra_secure(supplier_uuid uuid)
RETURNS TABLE(
  id uuid,
  company_name text,
  contact_person text,
