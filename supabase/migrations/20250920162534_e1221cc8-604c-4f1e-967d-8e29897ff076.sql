-- CRITICAL SECURITY FIX: Driver Contact Information Protection
-- This migration protects driver phone numbers from unauthorized access

-- Step 1: Create secure view for deliveries without driver contact info
...
-- Step 2: Create ultra-secure function for driver contact access
...
-- Step 3: Create secure delivery listing function
...
-- Step 4: Create trigger to audit direct deliveries table access
CREATE OR REPLACE FUNCTION public.audit_deliveries_table_access()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
DECLARE
    current_user_role TEXT;
    sensitive_fields TEXT[] := ARRAY['driver_name', 'driver_phone'];
BEGIN
    ...
    -- Log access to deliveries table (especially if driver fields might be exposed)
    INSERT INTO delivery_access_log (
        user_id, resource_id, action, resource_type, sensitive_fields_accessed
    ) VALUES (
        auth.uid(),
        COALESCE(NEW.id, OLD.id),
        TG_OP || '_DELIVERIES_TABLE_DIRECT_ACCESS',
        'deliveries_table',
        CASE 
            WHEN NEW.driver_name IS NOT NULL OR NEW.driver_phone IS NOT NULL 
            THEN sensitive_fields
            ELSE ARRAY[]::TEXT[]
        END
    );
...
DROP TRIGGER IF EXISTS audit_deliveries_access_trigger ON public.deliveries;
CREATE TRIGGER audit_deliveries_access_trigger
    AFTER SELECT OR INSERT OR UPDATE OR DELETE ON public.deliveries
    FOR EACH ROW
    EXECUTE FUNCTION public.audit_deliveries_table_access();

-- Step 5: Update RLS policies to be more restrictive
-- Drop existing policy and replace with more secure version
DROP POLICY IF EXISTS "deliveries_role_based_access" ON public.deliveries;

-- Create new restrictive policy - only admins get full access, others get limited access
CREATE POLICY "deliveries_admin_full_access" ON public.deliveries
    FOR ALL 
    USING (
        EXISTS (
...
-- Policy for builders - can only see their deliveries, driver info restricted
CREATE POLICY "deliveries_builder_restricted_access" ON public.deliveries
    FOR SELECT 
    USING (
        EXISTS (
...
-- Policy for suppliers - can only see assigned deliveries, driver info restricted  
CREATE POLICY "deliveries_supplier_restricted_access" ON public.deliveries
    FOR SELECT 
    USING (
        EXISTS (
            SELECT 1 FROM profiles p
...
-- Policy for delivery status updates (builders and suppliers can update status/notes only)
CREATE POLICY "deliveries_status_updates_only" ON public.deliveries
    FOR UPDATE 
    USING (
        EXISTS (
            SELECT 1 FROM profiles p
...
-- Step 7: Log security improvement
INSERT INTO security_events (
    event_type, 
    severity, 
    details
) VALUES (
    'DRIVER_CONTACT_PROTECTION_IMPLEMENTED',
    'info',
    jsonb_build_object(
        'description', 'Driver phone numbers protected from unauthorized access',
