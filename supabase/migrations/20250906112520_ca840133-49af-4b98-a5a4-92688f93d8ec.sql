...
-- Add additional security audit logging for any provider access attempts
CREATE OR REPLACE FUNCTION log_provider_security_access()
RETURNS TRIGGER AS $$
BEGIN
    -- Log any attempt to access the delivery_providers table directly
    INSERT INTO provider_contact_security_audit (
        ...
        'DIRECT_TABLE_ACCESS_ATTEMPT',
        false,
        false,
        'Direct table access blocked - all provider access must go through secure functions',
        'critical'
    );
...
-- Create trigger to log all direct access attempts
DROP TRIGGER IF EXISTS provider_security_access_log ON delivery_providers;
CREATE TRIGGER provider_security_access_log
    BEFORE SELECT ON delivery_providers
    FOR EACH STATEMENT
    EXECUTE FUNCTION log_provider_security_access();
