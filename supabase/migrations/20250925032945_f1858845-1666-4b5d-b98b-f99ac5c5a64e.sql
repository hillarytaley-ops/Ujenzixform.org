-- ====================================================
-- DELIVERY PROVIDERS SECURITY LOCKDOWN
...
-- Drop existing policy and create ultra-secure ones
...
-- Create audit function for provider contact access
...
BEGIN
  ...
  INSERT INTO provider_contact_security_audit (
    ...
    TG_OP,
    (current_user_role = 'admin' OR auth.uid() = COALESCE(NEW.user_id, OLD.user_id)),
    CASE 
      WHEN current_user_role = 'admin' THEN format('Admin %s operation', TG_OP)
...
CREATE TRIGGER audit_delivery_providers_access
  ...
  FOR EACH ROW EXECUTE FUNCTION public.audit_provider_contact_access();
