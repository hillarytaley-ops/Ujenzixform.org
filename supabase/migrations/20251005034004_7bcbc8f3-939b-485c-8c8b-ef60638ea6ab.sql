...
DROP POLICY IF EXISTS "profiles_block_anonymous" ON profiles;
...
-- Profile owner can view and update their own profile ONLY
CREATE POLICY "profiles_strict_owner_access"
ON profiles FOR ALL
TO authenticated
...
CREATE POLICY "profiles_strict_admin_access"
ON profiles FOR ALL
TO authenticated
...
CREATE POLICY "profiles_strict_block_anon"
ON profiles FOR ALL
TO anon
USING (false);
...
DROP VIEW IF EXISTS suppliers_public_directory;
CREATE VIEW suppliers_public_directory AS
SELECT 
  id,
  company_name,
  specialties,
...
DROP POLICY IF EXISTS "quotations_requester_own_only" ON quotation_requests;
DROP POLICY IF EXISTS "quotations_supplier_assigned_only" ON quotation_requests;

-- Requester can only view their own quotations
CREATE POLICY "quotations_strict_requester_only"
...
-- Supplier can ONLY view quotes where they are THE supplier (prevents competitor quote visibility)
CREATE POLICY "quotations_strict_supplier_assigned"
ON quotation_requests FOR SELECT
TO authenticated
USING (
  EXISTS (
...
CREATE POLICY "quotations_strict_admin_all"
ON quotation_requests FOR SELECT
TO authenticated
USING (has_role(auth.uid(), 'admin'::app_role));

-- Create audit table for quote access tracking
...
CREATE POLICY "quotation_audit_strict_admin"
ON quotation_access_audit FOR SELECT
TO authenticated
USING (has_role(auth.uid(), 'admin'::app_role));

-- System can insert audit logs
CREATE POLICY "quotation_audit_strict_insert"
