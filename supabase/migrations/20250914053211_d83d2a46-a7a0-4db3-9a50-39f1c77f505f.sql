...
-- 2. Create additional RLS policy to block GPS location data access
CREATE POLICY "delivery_providers_gps_location_restricted" ON delivery_providers
FOR SELECT USING (
  -- Block access to current_latitude/current_longitude unless specifically authorized
  EXISTS (
    ...
    AND (
      ...
      OR EXISTS (
        -- Only allow GPS access during active deliveries
        SELECT 1 FROM delivery_requests dr
...
-- 3. Create function to securely check provider availability without exposing exact locations
...
-- 4. Enhance existing function to detect and log suspicious access patterns
...
-- 5. Create view that only exposes safe provider information
...
INSERT INTO security_events (
  ...
  jsonb_build_object(
    'description', 'Enhanced protection for delivery provider personal information including contact masking, GPS location restrictions, and access monitoring',
    'security_measures', ARRAY[
      'contact_data_masking_function',
      'gps_location_restrictions', 
      'safe_provider_view',
