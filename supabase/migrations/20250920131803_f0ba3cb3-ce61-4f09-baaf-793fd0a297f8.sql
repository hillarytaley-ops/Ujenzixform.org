...
SELECT 
    ...
    CASE 
        ...
        ELSE 'Protected'
    END as contact_info,
    -- Completely hide sensitive data
    CASE 
        WHEN EXISTS (
            SELECT 1 FROM profiles p 
            WHERE p.user_id = auth.uid() 
        ...
        ELSE NULL
    END as email,
    -- Never expose addresses, document paths, or rates to unauthorized users
    CASE 
        WHEN EXISTS (
            SELECT 1 FROM profiles p 
            WHERE p.user_id = auth.uid() 
...
INSERT INTO emergency_security_log (
    ...
    auth.uid(),
    'CRITICAL_DELIVERY_PROVIDER_DATA_PROTECTION_IMPLEMENTED',
    'SECURITY FIX: Delivery provider personal information (phone, email, address, documents) now protected with strict RLS. Only admins and providers themselves can access sensitive data. Builders limited to active business relationships only.',
    'critical'
);
