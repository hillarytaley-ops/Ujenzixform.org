-- Fix Security Issues: Add Missing RLS Policies for Tables with RLS Enabled (Fixed)
...
-- 12. order_materials (fixed alias issue)
...
USING (
  EXISTS (
    ...
    WHERE delivery_order.id = order_materials.order_id
    AND (p.role = 'admin' OR p.id = delivery_order.builder_id OR p.id = delivery_order.supplier_id)
  )
)
WITH CHECK (
  EXISTS (
...
CREATE POLICY "Builders can manage their own projects"
ON public.projects FOR ALL
USING (
  EXISTS (
    SELECT 1 FROM profiles p
    WHERE p.user_id = auth.uid() 
    AND (p.role = 'admin' OR p.id = projects.builder_id)
  )
)
WITH CHECK (
  EXISTS (
...
CREATE POLICY "Users can view quotation requests they're involved in"
...
USING (
  EXISTS (
    ...
    AND (
      p.role = 'admin' OR
      p.id = quotation_requests.requester_id OR
      p.id = quotation_requests.supplier_id
    )
  )
);

CREATE POLICY "Builders can create quotation requests"
ON public.quotation_requests FOR INSERT
WITH CHECK (
  EXISTS (
    SELECT 1 FROM profiles p
    WHERE p.user_id = auth.uid() 
    AND p.id = quotation_requests.requester_id
    AND p.role = 'builder'
  )
);

CREATE POLICY "Suppliers can update quotation requests sent to them"
ON public.quotation_requests FOR UPDATE
USING (
  EXISTS (
    SELECT 1 FROM profiles p
    WHERE p.user_id = auth.uid() 
    AND p.id = quotation_requests.supplier_id
    AND p.role = 'supplier'
  )
);
...
INSERT INTO emergency_security_log (
  ...
  'Added missing RLS policies to 14 tables to prevent unauthorized data access',
  auth.uid()
);