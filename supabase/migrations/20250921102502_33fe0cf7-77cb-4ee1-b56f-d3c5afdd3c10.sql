-- EMERGENCY SUPPLIERS TABLE SECURITY FIX - COMPLETE REBUILD
...
CREATE POLICY "suppliers_admin_only_read_2024_new" ON public.suppliers
FOR SELECT USING (
  EXISTS (
...
CREATE POLICY "suppliers_admin_only_write_2024_new" ON public.suppliers
FOR ALL USING (
  EXISTS (
    SELECT 1 FROM profiles 
    WHERE user_id = auth.uid() AND role = 'admin'
...
BEGIN
  ...
  -- For non-admin users without business relationship, deny access
  RETURN jsonb_build_object(
...
-- Update secure supplier directory function to use new verification
CREATE OR REPLACE FUNCTION public.get_suppliers_ultra_secure()
RETURNS TABLE(
  id uuid, 
  company_name text, 
  specialties text[], 
...
BEGIN
  ...
  IF current_user_role = 'admin' THEN
    ...
    SELECT 
      ...
      'full_admin_access'::text as access_level
    FROM suppliers s
    ORDER BY s.company_name;
  ELSE
    -- Non-admin users get no access to suppliers directory
    RETURN;
  END IF;
END;
$$;

-- Update secure contact function with stricter time-limited access
...
VALUES (
  'SUPPLIERS_TABLE_COMPLETE_LOCKDOWN_FINAL',
  'Emergency ultra-secure suppliers contact protection - admin only access with time-limited business verification',
  jsonb_build_object(
    'protection_level', 'MAXIMUM_ULTRA_SECURE',
    'admin_only_table_access', true,
    'business_verification_required', true,
    'time_limited_access', true,
    'contact_harvesting_blocked', true,
