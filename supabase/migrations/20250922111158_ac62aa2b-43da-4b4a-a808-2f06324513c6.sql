-- First, let's create a comprehensive business relationship verification system
...
CREATE OR REPLACE FUNCTION public.get_suppliers_with_contact_protection()
RETURNS TABLE(
    id uuid,
    company_name text,
    specialties text[],
...
-- but protect contact information
...
-- Create audit trigger for contact access attempts
CREATE OR REPLACE FUNCTION audit_supplier_contact_access()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
DECLARE
    current_user_role text;
    accessed_contact_fields text[] := ARRAY[]::text[];
BEGIN
    -- Only audit when contact fields are being accessed
    ...
    -- Check which contact fields were accessed
    IF NEW.contact_person IS DISTINCT FROM OLD.contact_person THEN
        accessed_contact_fields := array_append(accessed_contact_fields, 'contact_person');
    END IF;
    IF NEW.email IS DISTINCT FROM OLD.email THEN
        accessed_contact_fields := array_append(accessed_contact_fields, 'email');
    END IF;
    IF NEW.phone IS DISTINCT FROM OLD.phone THEN
        accessed_contact_fields := array_append(accessed_contact_fields, 'phone');
    END IF;
    IF NEW.address IS DISTINCT FROM OLD.address THEN
        accessed_contact_fields := array_append(accessed_contact_fields, 'address');
    END IF;
    
    -- Log contact field modifications
    IF array_length(accessed_contact_fields, 1) > 0 THEN
        INSERT INTO supplier_contact_security_audit (
            user_id, supplier_id, contact_field_requested,
            business_relationship_verified, access_granted,
            access_justification, security_risk_level,
            ...
            auth.uid(), COALESCE(NEW.id, OLD.id), array_to_string(accessed_contact_fields, ','),
            has_legitimate_business_relationship(COALESCE(NEW.id, OLD.id)),
            (current_user_role = 'admin' OR has_legitimate_business_relationship(COALESCE(NEW.id, OLD.id))),
            format('Contact field modification: %s by %s', TG_OP, COALESCE(current_user_role, 'unknown')),
            CASE 
                WHEN current_user_role = 'admin' THEN 'low'
                WHEN has_legitimate_business_relationship(COALESCE(NEW.id, OLD.id)) THEN 'medium'
                ELSE 'high'
            END,
            accessed_contact_fields
        );
    END IF;
...
DROP TRIGGER IF EXISTS suppliers_contact_access_audit ON suppliers;
CREATE TRIGGER suppliers_contact_access_audit
    AFTER UPDATE ON suppliers
    FOR EACH ROW EXECUTE FUNCTION audit_supplier_contact_access();