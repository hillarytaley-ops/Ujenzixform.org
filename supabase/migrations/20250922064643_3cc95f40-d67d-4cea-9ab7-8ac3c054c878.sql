...
-- Create trigger function to audit write operations only
CREATE OR REPLACE FUNCTION public.audit_suppliers_write_operations()
RETURNS TRIGGER AS $$
BEGIN
  -- Log write operations (INSERT, UPDATE, DELETE)
  INSERT INTO public.suppliers_access_audit (
    user_id, supplier_id, access_type, access_granted,
    sensitive_fields_accessed, access_justification, security_risk_level
  ) VALUES (
...
-- Create trigger for write operations audit logging
CREATE TRIGGER suppliers_write_audit_trigger
  AFTER INSERT OR UPDATE OR DELETE ON suppliers
  FOR EACH ROW EXECUTE FUNCTION audit_suppliers_write_operations();
