...
DROP POLICY IF EXISTS "suppliers_audit_admin_read_only" ON public.suppliers_access_audit;
DROP POLICY IF EXISTS "suppliers_audit_system_insert" ON public.suppliers_access_audit;
DROP TRIGGER IF EXISTS suppliers_modifications_audit_trigger ON suppliers;
DROP FUNCTION IF EXISTS public.audit_suppliers_modifications();
DROP FUNCTION IF EXISTS public.verify_admin_suppliers_access(text, uuid, text[]);
DROP FUNCTION IF EXISTS public.get_suppliers_access_report(timestamp with time zone, timestamp with time zone);

-- Drop and recreate suppliers_access_audit table to ensure clean state
DROP TABLE IF EXISTS public.suppliers_access_audit CASCADE;

-- Create enhanced suppliers access audit table
CREATE TABLE public.suppliers_access_audit (
...
DROP POLICY IF EXISTS "suppliers_enhanced_admin_only_with_audit_2024" ON suppliers;
DROP POLICY IF EXISTS "suppliers_enhanced_admin_audit_2024" ON suppliers;

-- Create new enhanced suppliers policy with comprehensive audit logging
CREATE POLICY "suppliers_enhanced_admin_audit_2024" 
...
-- Create trigger function for INSERT/UPDATE/DELETE operations only
CREATE OR REPLACE FUNCTION public.audit_suppliers_modifications()
RETURNS TRIGGER AS $$
BEGIN
  -- Log modification operations with detailed field access
  INSERT INTO public.suppliers_access_audit (
    user_id, supplier_id, access_type, access_granted,
    sensitive_fields_accessed, access_justification, security_risk_level
  ) VALUES (
...
-- Create trigger for suppliers modifications
CREATE TRIGGER suppliers_modifications_audit_trigger
  AFTER INSERT OR UPDATE OR DELETE ON suppliers
  FOR EACH ROW EXECUTE FUNCTION audit_suppliers_modifications();
