...
BEGIN
  ...
  -- Build deliveries from delivery_requests + purchase_orders
  -- Categorize using material_items: scheduled / in_transit / delivered
  -- Exclude delivered (they use get_delivered_orders_for_provider)
  WITH provider_deliveries AS (
    SELECT DISTINCT
      COALESCE(dr.id, po.id) AS id,
    ...
    LEFT JOIN delivery_requests dr ON dr.purchase_order_id = po.id AND dr.provider_id = v_provider_id
    WHERE (po.delivery_provider_id = v_provider_id OR dr.id IS NOT NULL)
