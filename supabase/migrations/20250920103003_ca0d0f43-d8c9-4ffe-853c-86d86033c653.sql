...
-- Create trigger to log supplier modification attempts (INSERT, UPDATE, DELETE only)
CREATE OR REPLACE FUNCTION log_supplier_modifications()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
...
-- Apply the logging trigger for modifications only
DROP TRIGGER IF EXISTS suppliers_modification_audit_trigger ON public.suppliers;
CREATE TRIGGER suppliers_modification_audit_trigger
