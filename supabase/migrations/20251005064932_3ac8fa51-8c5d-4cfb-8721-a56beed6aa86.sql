...
SELECT 
  ...
  d.created_at,
  d.updated_at,
  -- Protect driver contact - only during active deliveries for authorized users
  CASE 
...
SELECT 
  p.id,
  -- Mask names unless admin or verified business relationship
  CASE 
  ...
  p.created_at,
  CASE 
    WHEN has_role(auth.uid(), 'admin'::app_role) THEN 'Available'
    ELSE 'Protected'
  END as business_registration_status
FROM profiles p
...
-- First create the logging function outside DO block
...
-- Now update the vault table policies
DO $$
BEGIN
...
VALUES (
  ...
  jsonb_build_object(
    ...
    'objects', ARRAY['deliveries_safe', 'profiles_business_directory', 'delivery_provider_personal_data_vault'],
    'measures', ARRAY['driver_contact_masked', 'business_relationship_verified', 'vault_access_logged'],
    'timestamp', NOW()
  )
);