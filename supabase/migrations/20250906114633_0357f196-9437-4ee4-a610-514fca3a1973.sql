...
-- Create a function to log provider access attempts on modifications
CREATE OR REPLACE FUNCTION log_provider_modification_attempt()
RETURNS TRIGGER AS $$
BEGIN
    -- Log modification attempts to the delivery_providers table
    INSERT INTO provider_contact_security_audit (
        ...
        CASE 
            ...
            ) THEN 'Admin modifying provider data'
            WHEN EXISTS (
                SELECT 1 FROM profiles p
                WHERE p.user_id = auth.uid() AND p.id = COALESCE(NEW.user_id, OLD.user_id)
            ) THEN 'Provider modifying own data'
            ELSE 'UNAUTHORIZED: Provider modification blocked'
        END,
        CASE 
            WHEN EXISTS (
                SELECT 1 FROM profiles p
...
-- Create trigger for modification logging
DROP TRIGGER IF EXISTS provider_modification_log ON delivery_providers;
CREATE TRIGGER provider_modification_log
    AFTER INSERT OR UPDATE OR DELETE ON delivery_providers
    FOR EACH ROW
    EXECUTE FUNCTION log_provider_modification_attempt();