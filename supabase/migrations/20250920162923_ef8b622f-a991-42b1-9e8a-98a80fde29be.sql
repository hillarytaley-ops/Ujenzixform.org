-- CRITICAL SECURITY FIX: Driver Contact Information Protection (Final Corrected)
...
-- Step 4: Update RLS policies - Drop ALL existing policies first
DROP POLICY IF EXISTS "deliveries_role_based_access" ON public.deliveries;
DROP POLICY IF EXISTS "deliveries_admin_full_access" ON public.deliveries;
DROP POLICY IF EXISTS "deliveries_builder_restricted_access" ON public.deliveries;
DROP POLICY IF EXISTS "deliveries_supplier_restricted_access" ON public.deliveries;
DROP POLICY IF EXISTS "deliveries_status_updates_only" ON public.deliveries;

-- Create new ultra-secure policies
-- Only admins get full access to all fields including driver info
CREATE POLICY "deliveries_admin_only_full_access" ON public.deliveries
    FOR ALL 
    USING (
        EXISTS (
...
-- Builders can see their deliveries but not driver contact info (use secure functions instead)
CREATE POLICY "deliveries_builder_limited_access" ON public.deliveries
    FOR SELECT 
    USING (
        EXISTS (
...
-- Suppliers can see assigned deliveries but not driver contact info (use secure functions instead)  
CREATE POLICY "deliveries_supplier_limited_access" ON public.deliveries
    FOR SELECT 
    USING (
        EXISTS (
            SELECT 1 FROM profiles p
...
CREATE POLICY "deliveries_limited_updates" ON public.deliveries
    FOR UPDATE 
    USING (
        EXISTS (
            SELECT 1 FROM profiles p
