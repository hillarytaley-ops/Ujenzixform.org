...
BEGIN
  ...
  INSERT INTO supplier_contact_security_audit (
    ...
    CASE 
      ...
      ELSE NULL
    END,
    format('%s_table_modification_%s', table_name, TG_OP),
...
-- Apply contact modification logging to both tables (INSERT, UPDATE, DELETE only)
DROP TRIGGER IF EXISTS log_supplier_contact_modifications ON suppliers;
CREATE TRIGGER log_supplier_contact_modifications
  AFTER INSERT OR UPDATE OR DELETE ON suppliers
  FOR EACH ROW EXECUTE FUNCTION log_contact_modifications();

DROP TRIGGER IF EXISTS log_provider_contact_modifications ON delivery_providers;  
CREATE TRIGGER log_provider_contact_modifications
