...
-- Public can only see basic business info (no contact data)
...
-- 9. Log all contact data access attempts
...
-- 10. CREATE audit trigger function for contact access
...
BEGIN
  ...
  INSERT INTO contact_access_audit (
    user_id, target_table, target_record_id, 
...
CREATE TRIGGER audit_profiles_contact_access
AFTER SELECT ON public.profiles
FOR EACH ROW EXECUTE FUNCTION audit_contact_access();

CREATE TRIGGER audit_suppliers_contact_access  
AFTER SELECT ON public.suppliers
FOR EACH ROW EXECUTE FUNCTION audit_contact_access();

CREATE TRIGGER audit_providers_contact_access
