-- ULTRA SECURE DELIVERY PROVIDERS - ADMIN ONLY ACCESS
...
-- 8. Create trigger to log all delivery provider access attempts
CREATE OR REPLACE FUNCTION public.log_provider_access_attempts()
...
-- Attach trigger to delivery_providers table
DROP TRIGGER IF EXISTS log_provider_access ON public.delivery_providers;
CREATE TRIGGER log_provider_access
  BEFORE SELECT OR INSERT OR UPDATE OR DELETE ON public.delivery_providers
  FOR EACH ROW EXECUTE FUNCTION public.log_provider_access_attempts();
