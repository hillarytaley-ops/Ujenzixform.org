-- CRITICAL SECURITY FIX: Supplier Contact Information Protection (Corrected)
-- This migration protects supplier contact information from unauthorized harvesting

-- Step 1: Drop existing functions with conflicting signatures
...
CREATE FUNCTION public.get_supplier_contact_secure(supplier_uuid uuid)
RETURNS TABLE(
    supplier_id uuid,
    company_name text,
    email text,
...
CREATE FUNCTION public.get_suppliers_directory_secure()
RETURNS TABLE(
    id uuid,
    company_name text,
    specialties text[],
...
-- Contact info requires active business relationship through secure functions
