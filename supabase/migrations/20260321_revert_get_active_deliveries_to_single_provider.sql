-- ============================================================
-- Revert get_active_deliveries_for_provider to single provider_id match
-- (20260320 OR auth.uid() caused Scheduled/In Transit to disappear)
-- Keeps auth.uid() fallback for v_provider_id when table is empty.
...
BEGIN
  ...
  -- Single provider match only (no OR auth.uid() - that made Scheduled/In Transit disappear)
  WITH from_dr AS (
...
COMMENT ON FUNCTION public.get_active_deliveries_for_provider() IS
  'Returns active (scheduled + in_transit) deliveries for current provider. Single provider_id match (reverted OR auth.uid() to restore Scheduled/In Transit data).';
