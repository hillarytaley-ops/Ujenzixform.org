-- CRITICAL SECURITY FIX: Supplier Contact Information Protection (Fixed)
...
-- Step 2: Drop existing vulnerable policies to replace with secure ones
DO $$ 
DECLARE 
    policy_rec RECORD;
BEGIN 
...
-- Step 3: Create secure public directory view without contact information
...
-- Step 4: Create ultra-secure function for supplier contact access
CREATE OR REPLACE FUNCTION public.get_supplier_contact_ultra_secure(supplier_uuid uuid)
RETURNS TABLE(
    supplier_id uuid,
    company_name text,
    email text,
...
DECLARE
    ...
    justification text := 'Access denied - no business relationship';
BEGIN
    -- Get supplier record
    ...
    ELSE
        justification := 'BLOCKED: No active business relationship - potential harvesting attempt';
    END IF;
    ...
    INSERT INTO supplier_contact_security_audit (
        ...
        auth.uid(), 
        supplier_uuid,
        'full_contact_ultra_secure_request',
        can_access_contact,
        can_access_contact,
        justification,
    ...
    RETURN QUERY SELECT
        ...
        CASE 
            ...
            ELSE 'Protected - Contact via platform'
        END,
        CASE 
...
-- Step 5: Create secure suppliers directory function
CREATE OR REPLACE FUNCTION public.get_suppliers_directory_ultra_secure()
RETURNS TABLE(
    id uuid,
    company_name text,
    specialties text[],
...
BEGIN
    ...
    -- Return secure supplier directory data (no contact info exposed)
    RETURN QUERY
    SELECT 
        s.id,
        s.company_name,
        ...
        END,
        -- Can request contact only with legitimate business need
        CASE 
            WHEN current_user_role = 'admin' THEN true
            WHEN current_user_role = 'builder' AND s.is_verified = true THEN true
            ELSE false
        END
    FROM suppliers s
...
-- Step 6: Create new ultra-secure RLS policies with unique names
-- Only admins get full access to all supplier data
CREATE POLICY "suppliers_2024_admin_only_access" ON public.suppliers
    FOR ALL 
    USING (
        EXISTS (
...
-- Suppliers can access their own data only
CREATE POLICY "suppliers_2024_owner_access" ON public.suppliers
    FOR ALL 
    USING (
        EXISTS (
...
-- All contact access must go through secure functions
CREATE POLICY "suppliers_2024_builder_directory_only" ON public.suppliers
    FOR SELECT 
    USING (
        EXISTS (
            SELECT 1 FROM profiles p
...
CREATE POLICY "suppliers_2024_delivery_blocked" ON public.suppliers
    FOR ALL 
    USING (false); -- Deny all access for delivery providers

-- Step 7: Grant appropriate permissions
GRANT SELECT ON public.suppliers_directory_public TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_supplier_contact_ultra_secure(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_suppliers_directory_ultra_secure() TO authenticated;