-- ============================================================
-- DELIVERY MILEAGE & PAY MIGRATION
-- Copy this entire file and paste into Supabase SQL Editor → Run
-- ============================================================
-- Adds: distance_km, delivery_mileage_config, haversine_km,
--       get_provider_mileage_pay, admin_get_all_providers_mileage_pay
-- ============================================================
...
-- 3. Haversine distance in km (lat/long)
...
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'delivery_requests' AND column_name = 'pickup_latitude'
  ) AND EXISTS (
    SELECT 1 FROM information_schema.columns
    WHERE table_name = 'delivery_requests' AND column_name = 'delivery_latitude'
  ) THEN
...
-- 6. RPC: Admin - all providers mileage/pay
