...
export function useDeliveriesUnified(): UseDeliveriesUnifiedResult {
  ...
  const fetchUnified = useCallback(async () => {
    ...
    // RPC DISABLED: Function is hanging/timing out. REST API fallback works fine.
    // Dashboard will use REST API from useDeliveryProviderData hook instead.
    console.log('⚠️ RPC disabled - dashboard using REST API (which works)');
    setScheduled([]);
    setInTransit([]);
    setDelivered([]);
    setLoading(false);
