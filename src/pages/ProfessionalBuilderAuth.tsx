/**
 * ProfessionalBuilderAuth - BUILD v13 - CHECK SESSION FIRST
 */
...
console.log('🔐 ProfessionalBuilderAuth BUILD v13');
...
const ProfessionalBuilderAuth: React.FC = () => {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (authData.user) {
        await Promise.all([
          supabase.from('profiles').upsert({ id: authData.user.id, user_id: authData.user.id, email: email.trim(), full_name: fullName.trim(), company_name: companyName.trim() || null, phone: phone.trim() || null, location: location.trim() || null }),
