  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (signInResponse.ok && signInData.access_token && signInData.user) {
        // User already exists! Update their role to private_client
        console.log('🔐 User exists! Updating role to private_client...');
        ...
        if (currentRole === ROLE) {
          // Already a private_client - just redirect
          console.log('🔐 User is already a private_client, redirecting...');
      ...
      if (!response.ok || data.error) {
        ...
        setIsLoading(false);
        toast({ title: 'Registration failed', description: data.error_description || data.error || data.msg || 'Failed to create account', variant: 'destructive' });
        return;
      }
