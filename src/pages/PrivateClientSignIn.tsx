...
const PrivateClientSignIn = () => {
  ...
  useEffect(() => {
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      if (session?.user) {
        console.log('🔐 PrivateClientSignIn: Session found, checking DB role...');
        ...
        console.log('🔐 PrivateClientSignIn: DB role:', dbRole);
        
        if (dbRole === 'private_client') {
          ...
          if (dbRole === 'professional_builder') window.location.href = '/professional-builder-dashboard';
