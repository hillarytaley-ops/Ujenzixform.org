-- ============================================================
-- FIX: RPC function to include all 100 orders (not just 59)
-- The issue is likely that some orders are being filtered by status
-- or the provider_id matching logic is too strict
-- ============================================================

-- Step 1: Check what statuses the 41 missing orders have
SELECT 
  'Step 1: Status Analysis of Missing Orders' as section,
...
GROUP BY dr.status, po.status
ORDER BY count DESC;

-- Step 2: Update RPC function to be less restrictive
-- The current RPC filters out orders with po.status IN ('cancelled', 'rejected', 'quote_rejected')
-- But it should only filter 'cancelled' for delivery_requests, and be more lenient with purchase_orders
...
BEGIN
  ...
  -- Final fallback: use auth.uid() so orders linked by user id still show
  ...
  -- FIX: Be less restrictive with purchase_orders status - only exclude truly cancelled orders
  WITH from_dr AS (
    ...
    WHERE (dr.provider_id = v_provider_id OR dr.provider_id = auth.uid())
      ...
      -- FIX: Only exclude purchase_orders that are truly cancelled, not 'rejected' or 'quote_rejected'
      -- These statuses don't prevent delivery - they're just quote-related
      AND (po.id IS NULL OR po.status NOT IN ('cancelled'))
  ),
  from_po AS (
    ...
    WHERE (
      ...
      -- FIX: Only exclude truly cancelled purchase_orders, not 'rejected' or 'quote_rejected'
...
SELECT 
  'Step 3: Verification After Fix' as section,
