-- ============================================================
-- APPLY FIX: Remove restrictive filters from RPC function
-- This will make all 100 orders appear consistently
-- ============================================================
-- Run this script in Supabase SQL Editor to apply the fix
-- ============================================================
...
BEGIN
  ...
  -- ✅ FIX APPLIED: Removed restrictive 'rejected' and 'quote_rejected' filters
  -- Only exclude truly 'cancelled' orders
  WITH from_dr AS (
...
COMMENT ON FUNCTION public.get_deliveries_for_provider_unified() IS
  'Single source for Delivery Dashboard: scheduled + in_transit + delivered. Uses material_items scan counts (aligned with Supplier QR/Orders). ✅ FIXED: Removed restrictive rejected/quote_rejected filters to show all 100 orders consistently.';

-- Verification message
SELECT 
  '✅ Fix Applied Successfully!' as status,
  'The RPC function now only filters out cancelled orders' as change,
  'All 100 orders will appear consistently after refresh' as result,
  'Test in the app (logged in) to see all orders' as next_step;
