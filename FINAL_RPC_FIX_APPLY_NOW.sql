-- ============================================================
-- FINAL FIX: Ensure RPC function is correctly updated
-- This will drop and recreate the function with the correct filters
-- ============================================================

-- Step 1: Drop the function to ensure clean recreation
DROP FUNCTION IF EXISTS public.get_deliveries_for_provider_unified();

-- Step 2: Recreate with the fix (only filter cancelled, not rejected/quote_rejected)
...
BEGIN
  ...
  -- ✅ CRITICAL FIX: Only filter 'cancelled', NOT 'rejected' or 'quote_rejected'
  WITH from_dr AS (
    ...
    WHERE (dr.provider_id = v_provider_id OR dr.provider_id = auth.uid())
      ...
      -- ✅ FIX: Only exclude cancelled, NOT rejected/quote_rejected
...
SELECT 
  ...
  CASE 
    WHEN pg_get_functiondef(oid) LIKE '%po.status != ''cancelled''%' 
      ...
      '✅ Fix confirmed - only filters cancelled'
    WHEN pg_get_functiondef(oid) LIKE '%po.status NOT IN (''cancelled'')%' 
      AND pg_get_functiondef(oid) NOT LIKE '%po.status NOT IN (''cancelled'', ''rejected'', ''quote_rejected'')%' THEN 
      '✅ Fix confirmed - only filters cancelled'
    ELSE 
      '❌ Fix not applied correctly - check function definition'
  END as verification
FROM pg_proc 
WHERE proname = 'get_deliveries_for_provider_unified';

-- Step 4: Show what should be returned
SELECT 
  '📊 Expected Results' as section,
  'After fix, RPC should return ~100+ orders' as note,
  'Check browser console (🔵 Unified RPC logs) to see actual count' as instruction;
